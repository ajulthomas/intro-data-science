...
quarter_summary_df <- act_weather_data %>% 
  ...
  mutate(
    wind_speed = as.numeric(`3pm_wind_speed_(km/h)`), 
    temperature = as.numeric(`3pm_Temperature`), 
    humidity = as.numeric(`3pm_relative_humidity_(%)`),
...
quarterly_data <-  quarter_summary_df %>% 
  gather(key = "attribute", value="avg_measure", min_temp:mean_humidity) %>% 
  separate(col = attribute, into = c("type", "variable"), sep = "_")
  # %>% spread(key = variable, value = avg_measure)

str(quarterly_data)

ggplot(quarterly_data) + 
  geom_line(aes(x=Quarter, y=avg_measure, colour=type), size=1) +
  facet_grid(~variable)


monthly_data <- monthly_summary_df %>% 
...
ggplot(monthly_data) + 
  geom_line(aes(x=Month, y=avg_measure, colour=type), size=1) +
